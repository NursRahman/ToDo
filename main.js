...
const renderToDo = ()=>{
    ...
    tasks.forEach(el =>{
        ...
        trash.addEventListener('click', ()=>{
            ...
            con ? tasks = tasks.filter(el=> el.id != id) : null
            addToLocal()
        })
        correct.addEventListener('click', ()=>{
            let cor = prompt('Show new record')
            tasks.forEach(el =>{
                if(el.id == id){
                    let p = confirm('Do oyu want to update?')
                    p ? el.name = cor : null
                }  
            })
            addToLocal()
...
//======================== JSON - text format obmena dannymi osnovannyi na JS ====================

// let obj = {
//     'bootcamp': 'ITC',
// }

// let objTrans = JSON.stringify(obj)

// alert(objTrans)

// objTrans=JSON.parse(objTrans)
